      ...
      <div className="max-w-[1440px] mx-auto px-5 sm:px-8 lg:px-12">
              ...
              <RevealOnScroll key={plan.slug || idx} delayMs={idx * 100}>
                <div
                  className={`relative flex flex-col justify-between p-8 sm:p-10 bg-[#FFFFFF] border transition-all duration-300 h-full shadow-[0_12px_40px_rgba(14,23,32,0.06)] ${
                    plan.featured
                      ? "border-[#17202B] ring-1 ring-[#17202B]"
                      : "border-[#DED7C9] hover:border-[#17202B]"
                  }`}
                >
                  {plan.featured ? (
                    <div className="absolute -top-3 right-8 bg-[#0F1725] text-[#F7F5EF] font-mono text-[10px] font-bold tracking-[0.2em] uppercase px-3 py-0.5">
                      MOST POPULAR
                    </div>
                  ) : null}

                  <div>
                    <div className="font-mono text-[10px] tracking-[0.2em] text-[#394352] uppercase mb-2 font-semibold">
                      {plan.billing_type}
                    </div>

                    <h2 className="font-editorial text-2xl sm:text-3xl text-[#17202B] mb-3">
                      {plan.name}
                    </h2>

                    <p className="font-sans text-xs sm:text-sm text-[#394352] font-light leading-relaxed mb-6">
                      {plan.description}
                    </p>

                    <div className="p-4 bg-[#F1EDE3] border border-[#DED7C9] mb-8">
                      <div className="font-mono text-xs text-[#394352] uppercase tracking-wide">Model Structure</div>
                      <div className="font-editorial text-2xl text-[#17202B] mt-1 font-bold">{plan.price}</div>
                    </div>

                    <div className="space-y-3 mb-8">
                      <div className="font-mono text-[10px] tracking-[0.2em] text-[#17202B] uppercase font-bold">
                        INCLUSIONS
                      </div>
                      {featuresList.map((feat: string) => (
                        <div key={feat} className="flex items-start gap-2 text-xs font-sans text-[#394352] leading-relaxed">
                          <span className="text-[#17202B] mt-0.5 shrink-0 font-bold">✔</span>
                          <span>{feat}</span>
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="pt-6 border-t border-[#DED7C9]">
                    <Button
                      href="/contact"
        ...
        <RevealOnScroll>
          <div className="mb-24">
            ...
            <div className="hidden lg:block bg-[#FFFFFF] border border-[#DED7C9] shadow-[0_12px_40px_rgba(14,23,32,0.06)]">
              <table className="w-full text-left font-sans text-xs">
                <thead>
                  <tr className="border-b border-[#DED7C9] bg-[#F1EDE3] font-mono text-xs text-[#17202B] uppercase font-bold">
                    <th className="p-5 w-2/5">Capabilities & Inclusions</th>
                    <th className="p-5 text-[#17202B]">Fixed-Scope Build</th>
                    <th className="p-5 text-[#17202B]">Dedicated Pod</th>
                    <th className="p-5 text-[#17202B]">Technical Advisory</th>
                  </tr>
                </thead>
                <tbody>
                  {COMPARISON_DATA.map((section) => (
                    <React.Fragment key={section.category}>
                      <tr className="bg-[#F1EDE3] border-t border-b border-[#DED7C9]">
                        <td colSpan={4} className="p-3.5 px-5 font-mono text-[11px] text-[#17202B] uppercase font-bold">
                          {section.category}
                        </td>
                      </tr>
                      {section.features.map((f, i) => (
                        <tr key={f.name} className={`border-b border-[#DED7C9] ${i % 2 === 0 ? "bg-transparent" : "bg-[#F1EDE3]/50"}`}>
                          <td className="p-4 px-5 text-[#17202B] font-semibold">{f.name}</td>
                          <td className="p-4 text-[#394352]">{f.fixed}</td>
                          <td className="p-4 text-[#17202B] font-bold">{f.pod}</td>
                          <td className="p-4 text-[#394352]">{f.advisory}</td>
                        </tr>
                      ))}
                    </React.Fragment>
                  ))}
            ...
            <div className="lg:hidden space-y-6">
              {COMPARISON_DATA.map((section) => (
                <div key={section.category} className="p-6 bg-[#FFFFFF] border border-[#DED7C9]">
                  <h3 className="font-mono text-xs text-[#17202B] uppercase font-bold mb-4 pb-2 border-b border-[#DED7C9]">
                    {section.category}
                  </h3>
                  <div className="space-y-4">
                    {section.features.map((f) => (
                      <div key={f.name} className="p-3 bg-[#F1EDE3] border border-[#DED7C9] space-y-2">
                        <div className="font-sans text-xs font-bold text-[#17202B]">{f.name}</div>
                        <div className="grid grid-cols-3 gap-2 font-mono text-[10px] pt-1">
                          <div>
                            <span className="text-[#68717B] block font-semibold">FIXED</span>
                            <span className="text-[#394352]">{f.fixed}</span>
                          </div>
                          <div>
                            <span className="text-[#17202B] block font-bold">POD</span>
                            <span className="text-[#17202B] font-bold">{f.pod}</span>
                          </div>
                          <div>
                            <span className="text-[#68717B] block font-semibold">ADVISORY</span>
                            <span className="text-[#394352]">{f.advisory}</span>
                          </div>
                        </div>
                      </div>
                    ))}
        ...
        {/* Guarantees */}
        <RevealOnScroll>
          <div className="p-10 sm:p-14 bg-[#FFFFFF] border border-[#DED7C9] shadow-[0_12px_40px_rgba(14,23,32,0.06)] grid grid-cols-1 md:grid-cols-3 gap-8">
            <div>
              <div className="font-mono text-xs text-[#17202B] font-bold mb-2">100% IP OWNERSHIP</div>
            ...
            <div>
              <div className="font-mono text-xs text-[#17202B] font-bold mb-2">MILESTONE ACCOUNTABILITY</div>
              <p className="font-sans text-xs sm:text-sm text-[#394352] font-light leading-relaxed">
                We tie financial milestones directly to verified code deliveries, staging demos, and automated test pass rates.
              </p>
            </div>
            <div>
              <div className="font-mono text-xs text-[#17202B] font-bold mb-2">POST-LAUNCH WARRANTY</div>
              <p className="font-sans text-xs sm:text-sm text-[#394352] font-light leading-relaxed">
